...
NSString * Canniff_Philip_SoundCloudShuffle$ModuleRequireFormat = @"(function(exports){"
...
@implementation Canniff_Philip_SoundCloudShuffleObject
...
-(void)dealloc
{
	...
	RELEASE_TO_NIL(_canniff_philip_soundcloudshuffle);
	OSSpinLockLock(&krollBridgeRegistryLock);
...
{
	[context gc];
	[_canniff_philip_soundcloudshuffle gc];
}
...
{
	// create Canniff_Philip_SoundCloudShuffle global object
    NSAutoreleasePool* pool = [[NSAutoreleasePool alloc] init];
    
    // Load the "Canniff_Philip_SoundCloudShuffle" object into the global scope
	NSString *basePath = (url==nil) ? [TiHost resourcePath] : [[[url path] stringByDeletingLastPathComponent] stringByAppendingPathComponent:@"."];
	_canniff_philip_soundcloudshuffle = [[Canniff_Philip_SoundCloudShuffleObject alloc] initWithContext:kroll host:host context:self baseURL:[NSURL fileURLWithPath:basePath]];
	
	TiContextRef jsContext = [kroll context];
	TiValueRef tiRef = [KrollObject toValue:kroll value:_canniff_philip_soundcloudshuffle];
	
	NSString *_canniff_philip_soundcloudshuffleNS = [NSString stringWithFormat:@"T%sanium","it"];
	TiStringRef prop = TiStringCreateWithCFString((CFStringRef) _canniff_philip_soundcloudshuffleNS);
	...
	if (preload!=nil)
	{
		...
		{
			KrollObject *ti = (KrollObject*)[_canniff_philip_soundcloudshuffle valueForKey:name];
...
-(id)loadCommonJSModule:(NSString*)code withSourceURL:(NSURL *)sourceURL
{
	...
	NSString *js = [[NSString alloc] initWithFormat:Canniff_Philip_SoundCloudShuffle$ModuleRequireFormat, dirname, filename,code];
...
loadNativeJS:
	...
	if (data!=nil)
	{
		...
		if (![wrapper respondsToSelector:@selector(replaceValue:forKey:notification:)]) {
			...
			@throw [NSException exceptionWithName:@"org.canniff_philip_soundcloudshuffle.kroll" 
	...
	@throw [NSException exceptionWithName:@"org.canniff_philip_soundcloudshuffle.kroll" reason:[NSString stringWithFormat:@"Couldn't find module: %@",path] userInfo:nil];
}
